...
app.get("/generateProof", (_, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        // Verify the proof
        const isValid = yield Reclaim.verifySignedProof(proof);
        if (!isValid) {
            return res.status(400).send("Proof is invalid");
        }
        // Transform the proof data to be used on-chain (for the contract)
        const proofData = yield Reclaim.transformForOnchain(proof);
